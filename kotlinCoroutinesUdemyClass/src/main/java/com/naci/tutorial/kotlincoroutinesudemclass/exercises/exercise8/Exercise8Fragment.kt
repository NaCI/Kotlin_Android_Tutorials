        ...
        btnFetch.setOnClickListener {
            ...
            coroutineScope.launch {
                try {
                    ...
                    // REMINDER : withContext(NonCancellable) block guarantees that covered code runs
                    // even coroutine cancelled. In that case "updateElapsedTimeJob.cancelAndJoin()"
                    // method throws cancellable exception on coroutineScope job cancelled
                    withContext(NonCancellable) {
                        updateElapsedTimeJob.cancelAndJoin()
                        txtElapsedTime.text = ""
                    }
                } finally {
                    // REMINDER : Always use NonCancellable Context on must-run actions
                    withContext(NonCancellable) {
